 ...
 */
@Component({
  selector: 'candidaturanonidonea',
  templateUrl: 'candidaturanonidonea.html'
})
export class CandidaturanonidoneaComponent {
  ...
  constructor(private db: AngularFireDatabase, public navCtrl: NavController,public navParams: NavParams) {
      ...
      this.db.list('/candidature/candidature_non_idonee', ref => ref.orderByChild('id_candidato').equalTo(this.keyUtente)).snapshotChanges().map(actions => {
